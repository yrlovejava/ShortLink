...
public class ShortLinkStatsSaveConsumer implements StreamListener<String, MapRecord<String,String,String>> {
    ...
    private void actualSaveShortLinkStats(ShortLinkStatsRecordDTO statsRecord) {
        ...
        rLock.lock();
        try {
            // 使用AOPContext 获取代理对象来调用saveShortLinkStats 方法
            ShortLinkStatsSaveConsumer proxy = (ShortLinkStatsSaveConsumer) AopContext.currentProxy();
            proxy.saveShortLinkStats(statsRecord,fullShortUrl);
        } finally {
            rLock.unlock();
        }
    }
    ...
    /**
     * 在数据库中保存短链接监控数据
     * @param statsRecord 短链接记录参数
     * @param fullShortUrl 完整短链接
     */
    @Transactional(rollbackFor = Exception.class)
    public void saveShortLinkStats(ShortLinkStatsRecordDTO statsRecord, String fullShortUrl) {
